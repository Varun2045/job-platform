...
export class StatsReporter {
  ...
  public static calculate(companies: CompanyConfig[]): SystemStats {
    ...
    const failureRate = totalScrapes > 0
      ? Number(((totalFailures / totalScrapes) * 100).toFixed(1))
      : 0;
    ...
    const avgResponseTimeMs = companiesWithStats.length > 0
      ? Math.round(companiesWithStats.reduce((acc, c) => acc + (c.avg_response_time_ms ?? 0), 0) / companiesWithStats.length)
      : 0;
