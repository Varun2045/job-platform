...
export class AssistantChatService {
  public static async answerQuery(userId: string, queryText: string, storage: StorageProvider): Promise<ChatResponse> {
    try {
      ...
      if (
        queryLower.includes('jobs should i apply') ||
        queryLower.includes('apply to today') ||
        queryLower.includes('which jobs')
      ) {
        const brief = await DailyBriefService.compileDailyBrief(userId, storage);
        if (brief.bestOpportunities.length === 0) {
          return {
            answer:
              'No specific high-score opportunities were scanned today. I recommend setting up new watchlists under Career Settings.',
            suggestedPrompts: ['What should I study this week?', 'Show companies hiring developers.'],
          };
        }
        const jobsList = brief.bestOpportunities
          .map((o) => `- **${o.title}** at **${o.company}** (Score: ${o.score}%)`)
          .join('\n');
        return {
      ...
      if (
        queryLower.includes('study this week') ||
        queryLower.includes('what should i study') ||
        queryLower.includes('learning roadmap')
      ) {
        ...
        const skillsList = roadmap.missingSkills
          .map((s) => `- **${s.skill}** (Priority: ${s.priority}, Estimated effort: ${s.effortWeeks} weeks)`)
          .join('\n');
        return {
      ...
      return {
        answer:
          'Hello! I am your Career Copilot. I can suggest jobs to apply to, identify skill gaps in your resume, simulate coding/system design mock interviews, or suggest follow-up emails.',
        suggestedPrompts: [
