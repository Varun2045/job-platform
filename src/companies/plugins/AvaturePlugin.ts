...
export class AvaturePlugin implements ScraperPlugin {
  ...
  public supports(company: CompanyConfig): boolean {
    return company.detected_ats === 'avature';
  }
  ...
  public async discover(company: CompanyConfig, httpClient: HttpClient): Promise<RawJob[]> {
    ...
    // Extract company slug from Avature URLs
    if (companySlug.startsWith('http://') || companySlug.startsWith('https://')) {
      const match = companySlug.match(/avature\.com\/([^/?#]+)/);
      if (match) {
        companySlug = match[1];
      } else {
        // Try subdomain pattern
        const subdomainMatch = companySlug.match(/:\/\/([^.]+)\.avature\.net/);
        if (subdomainMatch) {
    ...
    // Avature API endpoint pattern
    const url = `https://api.avature.com/${companySlug}/jobs`;

    Logger.debug(`Avature discovery request for company: ${company.name} [Slug: ${companySlug}]`);
    
    try {
      const response = await httpClient.get<any>(url);

      if (!response.data || !Array.isArray(response.data.jobs || response.data)) {
        throw new Error(`Unexpected Avature API response structure for ${company.name}`);
      }
      ...
      Logger.warn(`Avature API request failed for ${company.name}, falling back to generic scraping`);
      return [];
    }
  }

  public async enrich(rawJob: RawJob, _httpClient: HttpClient): Promise<RawJob> {
    // Avature already fetched full description in discover phase
    return rawJob;
  }
...
export const plugin = new AvaturePlugin();