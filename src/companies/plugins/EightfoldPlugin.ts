...
export class EightfoldPlugin implements ScraperPlugin {
  ...
  public supports(company: CompanyConfig): boolean {
    return company.detected_ats === 'eightfold';
  }
  ...
  public async discover(company: CompanyConfig, httpClient: HttpClient): Promise<RawJob[]> {
    ...
    // Extract company slug from Eightfold URLs
    if (companySlug.startsWith('http://') || companySlug.startsWith('https://')) {
      const match = companySlug.match(/eightfold\.ai\/([^/?#]+)/);
      if (match) {
        companySlug = match[1];
      } else {
        // Try subdomain pattern
        const subdomainMatch = companySlug.match(/:\/\/([^.]+)\.eightfold\.ai/);
        if (subdomainMatch) {
    ...
    // Eightfold API endpoint pattern
    const url = `https://api.eightfold.ai/v1/${companySlug}/jobs`;

    Logger.debug(`Eightfold discovery request for company: ${company.name} [Slug: ${companySlug}]`);
    
    try {
      const response = await httpClient.get<any>(url);

      if (!response.data || !Array.isArray(response.data.jobs || response.data)) {
        throw new Error(`Unexpected Eightfold API response structure for ${company.name}`);
      }
      ...
      Logger.warn(`Eightfold API request failed for ${company.name}, falling back to generic scraping`);
      return [];
    }
  }

  public async enrich(rawJob: RawJob, _httpClient: HttpClient): Promise<RawJob> {
    // Eightfold already fetched full description in discover phase
    return rawJob;
  }
...
export const plugin = new EightfoldPlugin();