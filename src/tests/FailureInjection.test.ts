...
describe('Failure-Injection and Resiliency Tests', () => {
  ...
  it('should continue orchestrator scrape run even if email notifier is offline', async () => {
    ...
    await storage.saveUserResume('failure-email-user', 'backend', 'TypeScript and Node.js backend developer skilled in Express and AWS.');
    ...
    const emailSpy = jest.spyOn(EmailNotificationProvider.prototype, 'sendDigest').mockRejectedValue(
