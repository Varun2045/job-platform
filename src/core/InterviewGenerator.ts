  ...
  public static generate(job: Job, profile: string): InterviewPrepResult {
    ...
    if (techStack.includes('react') || techStack.includes('next.js') || techStack.includes('nextjs')) {
      technicalQuestions.push(
        'Explain the lifecycle of React Server Components and their integration with SSR layouts.',
      );
    ...
    if (
      ...
      technicalQuestions.push(
        'Explain how to perform memory leak profiling on a running Express backend services process.',
      );
    }
    if (techStack.includes('java') || techStack.includes('spring boot') || techStack.includes('springboot')) {
      technicalQuestions.push(
        'Describe the Spring Bean lifecycle and how dependency injection profiles are configured.',
      );
      technicalQuestions.push(
        'Explain garbage collection options (e.g. G1GC, ZGC) and parameters to troubleshoot JVM heap sizes.',
      );
    }
    if (techStack.includes('typescript') || techStack.includes('javascript')) {
      technicalQuestions.push(
        'How does TypeScript resolve types structures? Explain conditional types and index signature access.',
      );
    }
    ...
    if (technicalQuestions.length < 3) {
      ...
      technicalQuestions.push(
        'Explain strategies for handling database transactions integrity across multiple decoupled services.',
      );
