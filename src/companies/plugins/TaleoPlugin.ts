...
export class TaleoPlugin implements ScraperPlugin {
  ...
  public supports(company: CompanyConfig): boolean {
    return company.detected_ats === 'taleo';
  }
  ...
  public async discover(company: CompanyConfig, httpClient: HttpClient): Promise<RawJob[]> {
    ...
    if (!url) {
      throw new Error(`No API endpoint configured for Taleo company ${company.name}`);
    }

    Logger.debug(`Taleo scraping: ${url}`);
    const response = await httpClient.get<string>(url);
    const $ = cheerio.load(response.data);

    const jobs: RawJob[] = [];

    // Taleo typically uses different CSS patterns
    $('.taleo-job, .job-item, [data-cid]').each((_, element) => {
      try {
        const $el = $(element);
        ...
        Logger.warn(`Failed to parse Taleo job item: ${error}`);
      }
    });

    if (jobs.length === 0) {
      Logger.warn(`No jobs found for ${company.name} via Taleo plugin`);
    }

    Logger.debug(`Taleo plugin found ${jobs.length} jobs for ${company.name}`);
    return jobs;
  }
  ...
  public async enrich(rawJob: RawJob, httpClient: HttpClient): Promise<RawJob> {
    try {
      ...
      Logger.warn(`Failed to enrich Taleo job ${rawJob.id}: ${error}`);
      return rawJob;
    }
  }
...
export const plugin = new TaleoPlugin();
