...
export class ResumeMatcher {
  ...
  private static readonly KNOWN_SKILLS = [
    'typescript', 'javascript', 'node.js', 'nodejs', 'nestjs', 'express', 'spring boot', 'springboot',
    'java', 'go', 'golang', 'postgresql', 'postgres', 'mongodb', 'redis', 'elasticsearch', 'aws',
    'amazon web services', 'gcp', 'google cloud', 'docker', 'kubernetes', 'microservices', 'rest api',
    'rest apis', 'graphql', 'distributed systems', 'system design', 'python', 'pytorch', 'tensorflow',
    'machine learning', 'deep learning', 'nlp', 'llm', 'transformers', 'rag', 'langchain', 'vector database',
    'c++', 'c#', 'rust', 'sql', 'nosql', 'react', 'next.js', 'nextjs', 'ci/cd', 'github actions'
  ];
  ...
  public static match(job: Job, profile: string): number {
    ...
    const skillsScore = requiredSkills.length > 0 
      ? (matchedSkillsCount / requiredSkills.length) * 100 
      : 80; // default to 80% if job description mentions no specific technical skills
    ...
    const isJuniorJob = /junior|early career|entry level|graduate|new grad|intern|associate|sde i\b/i.test(titleLower) ||
    ...
    const matchesTargetLocation = /india|bangalore|bengaluru|hyderabad|pune|noida|gurugram|mumbai|chennai/i.test(locationLower);
    ...
    Logger.debug(`Match score calculated for job [${job.company} - ${job.title}]: ${roundedScore}% (Skills: ${skillsScore.toFixed(0)}, Title: ${titleScore}, Exp: ${experienceScore}, Loc: ${locationScore}, Cosine: ${tfidfScore.toFixed(0)})`);
    return roundedScore;
  }

  public static explain(job: Job, profile: string): {
    overallScore: number;
