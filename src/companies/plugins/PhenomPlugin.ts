...
export class PhenomPlugin implements ScraperPlugin {
  ...
  public supports(company: CompanyConfig): boolean {
    return company.detected_ats === 'phenom';
  }
  ...
  public async discover(company: CompanyConfig, httpClient: HttpClient): Promise<RawJob[]> {
    ...
    const url = `https://api.phenom.com/v1/${companySlug}/jobs`;

    Logger.debug(`Phenom discovery request for company: ${company.name} [Slug: ${companySlug}]`);
    
    try {
      const response = await httpClient.get<any>(url);

      if (!response.data || !Array.isArray(response.data.jobs || response.data)) {
        throw new Error(`Unexpected Phenom API response structure for ${company.name}`);
      }
      ...
      Logger.warn(`Phenom API request failed for ${company.name}, falling back to generic scraping`);
      return [];
    }
  }

  public async enrich(rawJob: RawJob, _httpClient: HttpClient): Promise<RawJob> {
    // Phenom already fetched full description in discover phase
    return rawJob;
  }
...
export const plugin = new PhenomPlugin();