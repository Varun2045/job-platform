...
export class DailyReportGenerator {
  public static generate(
    ...
    const topCompanies = sortedComp.slice(0, 5).map(c => `- **${c.name}**: ${c.jobsFound} jobs found (Status: ${c.status})`);

    // Compute top scores
    const allMatches = [...addedMatches, ...updatedMatches].sort((a, b) => b.score - a.score);
    const topScores = allMatches.slice(0, 5).map(m => `- **${m.job.company}** - ${m.job.title} (**${m.score}%** match) [Link](${m.job.url})`);
    ...
    const fastest = [...durations].reverse().slice(0, 3).map(c => `- **${c.name}**: ${(c.durationMs / 1000).toFixed(2)}s`);
